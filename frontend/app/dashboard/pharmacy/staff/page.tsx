...
import { pharmacyStaffApi, type PharmacyStaff } from '@/lib/pharmacy'
import { 
  FiUsers, 
  FiEdit, 
  FiTrash2, 
...
export default function PharmacyStaffPage() {
  ...
  const loadStaff = useCallback(async (silent = false) => {
    ...
    const response = await pharmacyStaffApi.list()
    if (response.error) {
      showToast({
        type: 'error',
  ...
  // Open modal for Edit
  const handleOpenEditModal = (staff: PharmacyStaff) => {
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    if (isEditing && selectedStaff) {
      response = await pharmacyStaffApi.update(selectedStaff.id, payload)
    } else {
      response = await pharmacyStaffApi.create(payload)
    }
  ...
  const handleToggleStatus = async (staff: PharmacyStaff) => {
    ...
    const response = await pharmacyStaffApi.update(staff.id, { is_active: nextActive })
    if (response.error) {
      // Revert UI update
  ...
  const handleOpenDeleteModal = (staff: PharmacyStaff) => {
    setSelectedStaff(staff)
  ...
  const handleDeleteStaff = async () => {
    ...
    setSubmitting(true)
    const response = await pharmacyStaffApi.remove(selectedStaff.id)
    if (response.error) {
      showToast({
        type: 'error',
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-neutral-gray mt-1">
            Manage pharmacy accounts for your employees. They will only have access to view and manage orders.
          </p>
        </div>
        <Button 
          type="button" 
        ...
        <Card className="p-12 text-center border border-neutral-border">
          ...
          <p className="text-sm text-neutral-gray max-w-md mx-auto mb-6">
            {searchQuery.trim() 
              ...
              : 'Add staff accounts so your employees can view and process client orders.'}
          </p>
          {!searchQuery.trim() && (
        ...
        <form onSubmit={handleFormSubmit} className="space-y-4">
          ...
          <div className="space-y-1">
            ...
            <div className="relative">
              ...
              <input
                type="password"
                placeholder={selectedStaff ? "••••••••" : "Confirm password"}
                className={`w-full pl-10 pr-4 py-2.5 rounded-xl border bg-white text-neutral-dark text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all ${
                ...
                required={!selectedStaff || !!formPassword}
              />
            </div>
          ...
          <div className="py-2">
            <Toggle
              label="Active Status"
              description="Allow this staff member to log in and process orders."
      ...
      >
        <div className="space-y-4">
          <p className="text-sm text-neutral-dark">
            Are you sure you want to delete the staff account for <strong>{selectedStaff?.name}</strong>?
          </p>
